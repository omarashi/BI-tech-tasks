...
Write-Host "1) REST CRUD - GET /api/products (with token):"
$products = Invoke-RestMethod -Method Get -Uri 'http://localhost:5000/api/products' -Headers $h
...
Write-Host "`n4) POST /api/products (create) - expect 201 for Admin, 403 for User:"
try {
...
Write-Host "`n5) Security check - GET /api/products WITHOUT token (should fail):"
try {
    Invoke-RestMethod -Method Get -Uri 'http://localhost:5000/api/products' -ErrorAction Stop
